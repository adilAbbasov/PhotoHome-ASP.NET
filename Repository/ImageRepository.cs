...
namespace PhotoHome.Repository
{
	public class ImageRepository
	{
		...
		public async Task<List<Picture>> GetImage(int? pageNumber, string? searchPattern, string? searchType)
		{
			...
			{
				model = await _base.Images.Where(a => a.Allow == true).Include(a => a.Category).Where(a => a.Category.Name.ToLower().StartsWith(searchPattern.ToLower())).OrderByDescending(a => a.LikeCount).Skip(Convert.ToInt32(numberOfRecordToskip)).Take(pageSize).ToListAsync();
			}
